...
service = inject(MainDataSource);
  ...
  checkIfFormIsFilled(){
    if(this.cartService.cart$$.value.length == this.reservationService.temporaryReservedSeats$$.value.length&&this.reservationService.temporaryReservedSeats$$.value.length!==0){
      return false
    } else return true
  }
