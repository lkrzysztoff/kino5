...
import { Cart, seats } from './cart-interface';
...
removeFromCart(seat:string){
  this.cart$$.next(
    this.cart$$.value.filter(value => value.seat.position !== seat&&value.id !== seat)
  )
  console.log(this.cart$$.value)
}
...
doesCartHave(seat:string,showId:number){
    return this.cart$$.value.some(( value ) => value.seat.position == seat&&value.showId==showId);
}
